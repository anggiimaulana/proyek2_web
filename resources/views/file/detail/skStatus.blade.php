...
<head>
    ...
    <title>{{ $skStatus->nama }} - {{ $skStatus->pengajuan->kategoriPengajuan->nama_kategori }}</title>
</head>
...
<body>
    ...
    @php
        $bulanAngka = \Carbon\Carbon::parse($skStatus->pengajuan->created_at)->month;
        $bulanRomawi = [
            1 => 'I',
            2 => 'II',
            3 => 'III',
    ...
    <div class="judul">
        <p><b><u style="font-size: 14pt;">SURAT KETERANGAN STATUS</u></b>
            <br>Nomor: {{ $skStatus->pengajuan->id }}/Ds.2013/{{ $bulanRomawi }}/2025
        </p>
    </div>
    ...
    <table class="tabel-data">
        ...
        <tr>
            ...
            <td style="width: 60%">: {{ $skStatus->jenisKelaminPengaju->jenis_kelamin ?? '-' }}</td>
        </tr>
        <tr>
            <td style="width: 40%">Agama</td>
            <td style="width: 60%">: {{ $skStatus->agamaPengaju->nama_agama ?? '-' }}</td>
        </tr>
        <tr>
            <td style="width: 40%">Pekerjaan</td>
            <td style="width: 60%">: {{ $skStatus->pekerjaanPengaju->nama_pekerjaan ?? '-' }}</td>
        </tr>
        <tr>
            <td style="width: 40%">Alamat</td>
            <td style="width: 60%">: {{ $skStatus->alamat }}</td>
        </tr>
    </table>

    <p class="paragraf">
        Adalah benar warga desa kami yaitu Desa Bulak Lor Kecamatan Jatibarang Kabupaten Indramayu, Yang menurut data
        kami nama tersebut diatas berstatus
        <strong>{{ $skStatus->statusPerkawinanPengaju->status_perkawinan ?? '-' }}</strong>.
    </p>
    ...
    <div class="ttd">
        <p style="margin-top: 25">Bulak Lor,
            {{ \Carbon\Carbon::parse($skStatus->updated_at)->translatedFormat('d F Y') }} <br>Kuwu Desa
            Bulak Lor</p>
        @if ($qrCode)
