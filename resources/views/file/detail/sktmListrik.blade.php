...
<head>
    ...
    <title>{{ $sktmListrik->nama }} - {{ $sktmListrik->pengajuan->kategoriPengajuan->nama_kategori }}</title>
</head>
...
<body>
    ...
    @php
        $bulanAngka = \Carbon\Carbon::parse($sktmListrik->pengajuan->created_at)->month;
        $bulanRomawi = [
            1 => 'I',
            2 => 'II',
            3 => 'III',
    ...
    <div class="judul">
        <p><b><u style="font-size: 14pt;">SURAT KETERANGAN TIDAK MAMPU</u></b>
            <br>Nomor: {{ $sktmListrik->pengajuan->id }}/Ds.2013/{{ $bulanRomawi }}/2025
        </p>
    </div>
    ...
    <table class="tabel-data">
        ...
        <tr>
            ...
            <td style="width: 60%">: {{ $sktmListrik->nama }}</td>
        </tr>
        <tr>
            <td style="width: 40%">Umur</td>
            <td style="width: 60%">: {{ $sktmListrik->umur }} Tahun</td>
        </tr>
        <tr>
            <td style="width: 40%">Jenis Kelamin</td>
            <td style="width: 60%">: {{ $sktmListrik->jenisKelaminPengaju->jenis_kelamin ?? '-' }}</td>
        </tr>
        <tr>
            <td style="width: 40%">Agama</td>
            <td style="width: 60%">: {{ $sktmListrik->agamaPengaju->nama_agama ?? '-' }}</td>
        </tr>
        <tr>
            <td style="width: 40%">Pekerjaan</td>
            <td style="width: 60%">: {{ $sktmListrik->pekerjaanPengaju->nama_pekerjaan ?? '-' }}</td>
        </tr>
        <tr>
            <td style="width: 40%">Alamat</td>
            <td style="width: 60%">: {{ $sktmListrik->alamat }}</td>
        </tr>
    </table>
    ...
    <div class="ttd">
        <p style="margin-top: 25">Bulak Lor,
            {{ \Carbon\Carbon::parse($sktmListrik->updated_at)->translatedFormat('d F Y') }} <br>Kuwu Desa
            Bulak Lor</p>
        @if ($qrCode)
        ...
        @endif
        <p>
            <u><strong>{{ $sktmListrik->pengajuan->kuwuUpdated->nama }}</strong></u>
        </p>
    </div>
</body>
